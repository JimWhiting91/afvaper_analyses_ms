...
chr_sizes <- read.table("/Volumes/jimwhiting_external/Exeter/Genomes/STAR.chromosomes.release.fasta.fai")
...
window_outliers <- pbmclapply(wind_sizes,function(window_snps){
  chr_res <- lapply(chrs,function(chr){
    #  print(chr)
    return(readRDS(paste0("outputs/five_aside_test_v2_",chr,"_AF_eigen_res_windsize_",window_snps,".rds")))
  })
  ...
  signif <- signif_eigen_windows(allchr_eigen_res,cutoffs = all_null_cutoffs[,1])
  ...
  if(length(na.omit(eig1_signif)) > 0){
    ...
    five_aside_eigenvec1_window_summaries <- NULL
  }
  # Summarise eigenvector 2 windows...
  if(length(na.omit(eig2_signif)) > 0){
    ...
    five_aside_eigenvec2_window_summaries <- NULL
  }
  
  # Finally just fetch all the pvals...
  wind_pvals <- afvaper::eigen_pvals(allchr_eigen_res,all_nulls)
  
  return(list(med_wind_size,
              eig1_signif,
              eig2_signif,
              five_aside_eigenvec1_window_summaries,
              five_aside_eigenvec2_window_summaries,
              wind_pvals))
},mc.cores = 4)
...
overlapping_outliers <- lapply(list(eig1_outliers,eig2_outliers),function(outlier_regions){
  ...
  # Filter null
  wind_sizes_tmp <- wind_sizes[!sapply(eig_bed, is.null)]
  ...
  names(eig_bed_sort) <- wind_sizes_tmp
...
wind_out <- lapply(wind_sizes,function(wind){
  ...
  chr_res <- pbmclapply(chrs,function(chr){
    #  print(chr)
    return(readRDS(paste0("outputs/five_aside_test_v2_",chr,"_AF_eigen_res_windsize_",wind,".rds")))
  },mc.cores=6)
  ...
  # Plot eigenvalue 1
  tmp_plots <- afvaper::eigenval_plot(allchr_eigen_res[grep("chr",names(allchr_eigen_res))],
                                      null_vectors = all_nulls,
                                      plot.pvalues = T)
  
  # # Fetch pvals
  # tmp_pvals <- afvaper::eigen_pvals(allchr_eigen_res[grep("chr",names(allchr_eigen_res))],
  #                                   null_vectors = all_nulls)
  ...
  eig_plots <- lapply(1:2,function(eig){
    ...
    p1 <- tmp_plot +
      ...
      scale_y_continuous(breaks=1:4,labels = 1:4,limits = c(0,5))
...
for(i in 1:nrow(eig1_supp)){
  ...
  res_tmp <- which(wind_sizes == as.integer(snp_tmp))
  eig1_supp$empPvalue[i] <- window_outliers[[res_tmp]][[6]][window_tmp,1]
}

# Write these
eig1_supp <- eig1_supp[,c(colnames(eig1_supp)[1:4],"empPvalue",colnames(eig1_supp)[5:8])]
...
for(i in 1:nrow(eig2_supp)){
  ...
  res_tmp <- which(wind_sizes == as.integer(snp_tmp))
  eig2_supp$empPvalue[i] <- window_outliers[[res_tmp]][[6]][window_tmp,2]
}

# Write these
eig2_supp <- eig2_supp[,c(colnames(eig2_supp)[1:5],"empPvalue",colnames(eig2_supp)[6:9])]
...
##################################################
# # In depth look at chr15 and chr1
# Use 50 snp windows as largest window size
chr_res <- mclapply(chrs,function(chr){
  #  print(chr)
  return(readRDS(paste0("outputs/five_aside_test_v2_",chr,"_AF_eigen_res_windsize_50.rds")))
},mc.cores=6)
...
popmap_in <- data.frame(rbindlist(lapply(pops2,function(x){
  read.table(paste0("/Volumes/jimwhiting_external/Exeter/VCFs/",x,".popmap"))
})))
popmap_in$pop<-NA
...
# Plot the region AFs
chr15_region_geno <- genotype_plot(vcf="/Volumes/jimwhiting_external/Exeter/VCFs/five_aside_STAR_3033083_allchr_shapeit_beagle.vcf.gz",
                                   chr="chr15",
                                   start=5028361,
                                   end=5050793,
                                   popmap = popmap_in,
...
test_mat <- rbindlist(lapply(c("chr15:5028361-5039371","chr15:5039402-5050793"),function(chr15_wind){
  test_mat <- data.frame(allchr_eigen_res[[chr15_wind]]$eigenvecs)[,1:2]
  test_mat$pop <- rownames(test_mat)
  test_mat$pop <- gsub("Tac","Tacarigua",test_mat$pop)
  test_mat$window <- chr15_wind
  test_mat
}))
...
# chr15 plot together -----------------------------------------------------
pdf("figs/FigureSX_HPLP_guppies_chr15_example.pdf",width=14,height=8)
plot_grid(merged_chr15_region2,
          plot_grid(geno_trajectories,
                    loading_plot,
                    ncol=1,nrow=2,labels = c("B","C"),label_size = 30,align = "v",axis="tblr",
                    hjust=0.05),
...
chr15_A_mat <- rbindlist(lapply(c("chr15:5028361-5039371","chr15:5039402-5050793"),function(chr15_wind){
  tmp <- data.frame(allchr_eigen_res[[chr15_wind]]$A_matrix)
  tmp$pos <- as.integer(sapply(strsplit(rownames(tmp),"_"),'[[',2))
  tmp
}))

# Make a longform
A_mat_plot <- data.frame(score=c(chr15_A_mat$X1,chr15_A_mat$X2),
...
chr15_A_mat_plot <- ggplot(A_mat_plot,aes(x=pos,y=abs(score),colour=top5))+
  geom_point()+
...
# Plot the region AFs
chr1_region <- genotype_plot(vcf="/Volumes/jimwhiting_external/Exeter/VCFs/five_aside_STAR_3033083_allchr_shapeit_beagle.vcf.gz",
                              chr="chr1",
                              start=9791276,
                              end=9798829,
                              popmap = popmap_in,
                              ...
                              cluster=F,
                              snp_label_size = 1000000)
merged_chr1_region <- combine_genotype_plot(chr1_region,heights = c(1,4))
merged_chr1_region
...
pdf("figs/FigureSX_HPLP_guppies_chr1_example.pdf",width = 12,height=8)
cowplot::plot_grid(chr1_A_mat_plot,chr1_region$positions,
                   chr1_region$genotypes,
