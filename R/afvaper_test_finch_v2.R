...
for(run in run_names){
  ...
  for(chr in large_chr){
    ...
    for(wind_size in c(50,100,200,500)){
      ...
      AF_vectors <- calc_AF_vectors_v2(vcf = vcf_in,popmap = popmap_run,vectors = vector_run,n_cores = 6,window_size = wind_size,end_cutoff = wind_size/2)
      ...
      null_vectors <- calc_AF_vectors_v2(vcf = vcf_in,popmap = popmap_run,vectors = vector_run,n_cores = 6,window_size = wind_size,end_cutoff = wind_size/2,null_perms = chr_perms[chr])
      ...
      saveRDS(list(AF_vectors,null_vectors,eigen_res),
              paste0("outputs/",run_name,"_v2_chr",chr,"_windowsize_",wind_size,".rds"))
    }
  }
}
