...
chr_res <- lapply(chrs,function(chr){
  #  print(chr)
  return(readRDS(paste0("outputs/stickleback_rad_test_v2_",chr,"_AF_eigen_res_windsize_",window_snps,".rds")))
})
...
pval_hist <- ggplot(eig1_pvals,aes(eig1_p))+
  geom_histogram(bins=30)+
  geom_vline(xintercept = -log10(0.05),linetype="dashed",colour="red2")+
  geom_vline(xintercept = -log10(0.01),linetype="dotted",colour="black")+
  labs(y="Count",x=expression(-log[10](empirical~p)))+
  ggtitle("M-F Stickleback, 10 SNP, Eigenvalue 1")

# And see all that are over 2...
eig1_pvals[eig1_pvals$eig1_p > 2,]
nrow(eig1_pvals[eig1_pvals$eig1_p > 2,])/nrow(eig1_pvals)
nrow(eig1_pvals[eig1_pvals$eig1_p > -log10(0.05),])/nrow(eig1_pvals)
...
per_chrom_ridges <- ggplot(per_chrom_eig1,aes(y=chr_F,x=eig1))+
  ...
  geom_density_ridges(quantile_lines = TRUE,quantiles=2,fill="lightblue",rel_min_height = 0.01)+
...
windows_to_find <- eig1_winds_res$window_id
for(i in 1:length(new_chr_names)){
  windows_to_find <- gsub(paste0(names(new_chr_names)[i],":"),paste0(new_chr_names[i],":"),windows_to_find)
}
# Fetch pvals...
eig1_winds_res$empPvalue <- eigen_pvals(allchr_eigen_res[windows_to_find],all_nulls)[,1]
...
windows_to_find <- eig2_winds_res$window_id
for(i in 1:length(new_chr_names)){
  windows_to_find <- gsub(paste0(names(new_chr_names)[i],":"),paste0(new_chr_names[i],":"),windows_to_find)
}
# Fetch pvals...
eig2_winds_res$empPvalue <- eigen_pvals(allchr_eigen_res[windows_to_find],all_nulls)[,2]
