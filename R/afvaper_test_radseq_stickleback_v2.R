...
# Test inputs
stickleback_rad_test <- "/Volumes/jimwhiting_external/Nottingham/NatEvol_Ecol_Genomic_Parallelism/data/MxF_VCFs/Bc.vcf.gz"
wind_sizes <- c(10)
CORES <- 6
permutations <- 10000
...
for(chr in chrs){
  ...
  for(window_snps in wind_sizes){
    ...
    saveRDS(list(AF_input,null_input,eigen_res),
            paste0("outputs/stickleback_rad_test_v2_",chr,"_AF_eigen_res_windsize_",window_snps,".rds"))
  }
}
