...
# Test inputs
five_aside_test <- "/Volumes/jimwhiting_external/Exeter/VCFs/five_aside_STAR_3033083_final.vcf.gz"
five_aside_test_chr20 <- "/Volumes/jimwhiting_external/Exeter/VCFs/five_aside_STAR_chr20_scaf94_merged_shapeit_beagle.vcf.gz"
CORES <- 6
permutations <- 10000
wind_sizes <- c(25,100,200)
# wind_sizes <- c(50)
...
for(chr in chrs){
  ...
  for(window_snps in wind_sizes){
    ...
    AF_input <- calc_AF_vectors_v2(vcf = vcf_in,
    ...
    null_input <- calc_AF_vectors_v2(vcf = vcf_in,
                                  window_size = window_snps,
                                  popmap = test_popmap,
                                  vectors = test_vectors,
                                  n_cores = CORES,
    ...
    saveRDS(list(AF_input,null_input,eigen_res,null_cutoffs),
            paste0("outputs/five_aside_test_v2_",chr,"_AF_eigen_res_windsize_",window_snps,".rds"))
  }
}
